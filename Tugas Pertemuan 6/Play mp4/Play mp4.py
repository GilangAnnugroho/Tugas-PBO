...
class VideoPlayerApp:
    ...
    def play_video(self):
        if not self.video_path:
            return  # Tidak ada file video yang dipilih

        # Nonaktifkan tombol "Pilih Video" dan "Putar" selama pemutaran video
        ...
        if self.cap.isOpened():
            ...
            if ret:
                ...
                # Restart video dari awal jika sudah selesai
                self.cap.release()
                self.cap = cv2.VideoCapture(self.video_path)
                self.play_video()
        else:
            # Aktifkan kembali tombol setelah pemutaran video dihentikan
            self.select_button.config(state=tk.NORMAL)
            self.play_button.config(state=tk.NORMAL)

    def stop_video(self):
        # Menutup koneksi video dan mengaktifkan tombol setelah pemutaran video dihentikan
